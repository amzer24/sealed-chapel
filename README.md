...
assets/shaders/             cast_outline.gdshader + its shared ShaderMaterial
tools/gen_peat_atlas.py     Deterministic regenerator for assets/tiles/peat_atlas.png
```
...
- **Peat contrast: bone rim + soot-biased floor (this pass):** at the
  locked `CAMERA_ZOOM` of 2.5, the player and Crawler's bruise-purple
  silhouettes were reading low-contrast against the peat floor — sampling
  every silhouette-edge pixel against 4,000 random floor positions showed
  a mean luma gap of only ~34-36 (out of 255), with a worst case of
  literally 0 where the sprite's own soot-coloured shading pixels sat
  next to a soot floor pixel. The fix is a Godot-native `canvas_item`
  shader (`assets/shaders/cast_outline.gdshader`, applied via a single
  shared `ShaderMaterial` at `assets/shaders/cast_outline_material.tres`)
  on the `Sprite` node of `Player.tscn`, `Crawler.tscn` and `Wraith.tscn`:
  it paints bone `#E8DCC8` on any fully-transparent texel that touches an
  opaque one, sampling all 8 neighbours so the rim has no gap at a
  diagonal silhouette corner. Every sample goes through `TEXTURE`, which
  follows each Sprite2D's existing `texture_filter = NEAREST`, so the rim
  is a hard 1-texel ring with no blur — not a baked PNG redraw, so it
  applies uniformly to all three cast members from one resource. Re-running
  the same contrast sample with the outline in place lifts the mean gap to
  ~192-195 for all three, because bone is never close in value to any peat
  texel. Separately, `assets/tiles/peat_atlas.png` (regenerated by the new
  `tools/gen_peat_atlas.py`, fully deterministic) now biases soot `#1A1410`
  to roughly 68% of pixels (was ~46%, with rot `#2D1F18` the ~53% majority)
  so the floor reads darker overall; the sparse bone/curse grit scatter and
  the "all three density variants share one base dither field so no seam
  shows between tiles" property are both unchanged, and the fix is a
  probability-table tweak, not a new tiling algorithm. Only existing
  palette hexes are used anywhere — no sixth colour, no curse-coloured
  outline (that stays a Lee-only backup idea for later, not built here).
  Verified against the Shade contrast boards with a temporary, uncommitted
  harness scene instancing a real `Player` + `Crawler` + `Wraith` over a
  real peat `TileMapLayer` at `CAMERA_ZOOM`, screenshotted headlessly
  (`--rendering-driver opengl3` under Xvfb, Godot `4.7.2-stable` linux):
  player and Crawler now read clearly against the floor and the Wraith
  (already higher-contrast pre-outline at ~88 mean gap, all bone/curse)
  is still fully visible, not washed out by the extra rim. Also re-ran the
  plain `Main.tscn` headlessly (`--headless --path . --import` then
  `--quit-after 300`) with no runtime errors. Out of scope per the brief:
  a sixth colour, any blur/soft-edge outline, a full sprite redraw or
  PixelLab regen, HUD restyling, and the pre-existing periodic grit-dot
  pattern visible when many tiles of the same density variant repeat
  across a chunk (an artifact of the 3-fixed-variant atlas design itself,
  present before this pass too, not something a soot/rot ratio tweak
  changes).
